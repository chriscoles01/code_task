...
def main():
    ...
    segment_dictionary = add_count_to_segment_dict(segment_dictionary, original_regions)
...
def add_count_to_segment_dict(segment_dictionary : dict, original_regions : pd.DataFrame) -> pd.DataFrame:
    ...
    for i in range(len(sorted_keys)-1):
        ...
        if current_value != 0:
            if next_value == 0:
                ...
                lines.append(str(current_value) + '\t' + str(current_key) + '\t' + str(next_key) +'\n')
            else:
                # previous segment will end at start of next -1
                lines.append(str(current_value) + '\t' + str(current_key) + '\t' + str(next_key -1 ) +'\n')
    return lines
