...
class GenomeUtilities:
    ...
    def generate_constraint_graph(
            genome_DataFrame: pd.DataFrame) -> pd.DataFrame:
        ...
        merged = left.assign(key=1).merge(right.assign(key=1),
                                          on='key').drop('key', 1)

        merged['constraint'] = merged.apply(
            lambda row: GenomeUtilities.is_matched(row), axis=1)
