...
def test_get_solution():
    ...
    order_df = constraint_df.groupby(
    ...
    actual_constraint_solution = ConstraintSolver.get_solution(
        order_df.index, constraint_df, 3)
...
def test_generate_heuristic():
    ...
    actual_heuristic_solution = ConstraintSolver.generate_heuristic(
        constraint_df, order_df)
